...
if __name__ == "__main__":
    ...
    search_heuristic = ParameterGridSearch(
        T0_list=[10**i for i in np.linspace(6, 10, 5)],
        Tmin_list=[1],
        alpha_list=list(np.concatenate((np.linspace(.9995, .99, 4), np.linspace(.98, .82, 4)))),#list(np.linspace(.99, .82, 4)),
        max_iterations_list=[1], #list(np.linspace(550, 1650, 3)), #np.concatenate((np.linspace(100, 1000, 10), np.linspace(2000, 10000, 9))),
        epsilon_list=[1e-6, 1e-4, 1e-2, 1e-1],
        delta_list=[1e-4, 1e-3, 1e-2]
    )
    ...
    # data bases
    algo_res_database = DataBase("resultados_projeto1_exp6.csv")
    stat_res_database = DataBase("resultados_vrf_stat_exp6.csv")
    ...
    while NC <= 50:
        ...
        while not satisfactory_result:
            ...
            results = instrumented_DA_torch(data_vectors=data_vectors, NC=NC, T0=T0, Tmin=Tmin,
                                            ...
                                            target_D=final_D_criteria, seed=original_seed)
