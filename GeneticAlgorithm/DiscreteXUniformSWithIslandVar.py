...
class DiscreteXUniformSWithIslandVar(Recombination):
    ...
    def execute(self):
        ...
        for isl in range(self.n_islands):
            prob[isl] = np.around(((self.parents_tensor[:, 0] == isl).sum() / self.parents_tensor.shape[0]).item(), 6)

        prob_list = list(prob.values())/sum(list(prob.values()))

        child_idx_list = rd.choice(list(prob.keys()),
                                   self.offspring_tensor.shape[0],
                                   p=prob_list)
