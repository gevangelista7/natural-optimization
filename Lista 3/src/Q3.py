        ...
        for K in K_list:
            for eps in epsilon_list:
                for T0 in T0_list:
                    ...
                    results[('SA', N, K, eps, T0)] = (J_min_sa, x_min_sa, norm_x_sa, history_sa, elapsed_time_sa)

                    start_time_fsa = time.time()
                    x_min_fsa, J_min_fsa, history_fsa = fast_sa(calcJ=calcJ, x0=x0, N=N, K=K, epsilon=eps, T0=T0)
                    norm_x_fsa = np.linalg.norm(x_min_sa)
                    elapsed_time_fsa = time.time() - start_time_fsa
                    results[('FSA', N, K, eps, T0)] = (J_min_fsa, x_min_fsa, norm_x_fsa, history_fsa, elapsed_time_fsa)


                    print("{} tentativas realizadas, tempo total decorrido: {}".format(i, time.time() - general_start_time))
                    if norm_x_fsa < 0.3:
